...
function SearchResults() {
  ...
  const handleSubmit = (e) => { e.preventDefault(); doSearch(query); window.history.pushState({}, '', '/search?q=' + encodeURIComponent(query)) }
  ...
  const s = {
    ...
    sub: { fontFamily: '"Manrope", sans-serif', color: '#7a6355', marginBottom: '2rem' },
    form: { display: 'flex', gap: '0.75rem', marginBottom: '2.5rem' },
    input: { flex: 1, padding: '0.75rem 1rem', border: '1px solid #e8ddd4', backgroundColor: '#fff', fontFamily: '"Manrope", sans-serif', fontSize: '1rem', color: '#3d2314' },
  ...
  return (
    <div style={s.page}>
      <div style={s.wrap}>
        ...
        <p style={s.sub}>Search across recipes, songs, stories, remedies and more from across India.</p>
        <form style={s.form} onSubmit={handleSubmit}>
          <input style={s.input} value={query} onChange={e => setQuery(e.target.value)} placeholder='Try "rasam", "lullaby", "monsoon", "Coorg"...' />
          <button style={s.btn} type="submit">Search</button>
        </form>
        {loading && <div style={s.empty}>Searching...</div>}
        {!loading && searched && (
          <>
            <div style={s.count}>{total} result{total !== 1 ? 's' : ''} for &quot;{searchParams.get('q') || query}&quot;</div>
            {results.length === 0 ? (
              <div style={s.empty}>No memories found. Try a different search — or <a href="/share" style={{ color: '#c2714f' }}>share something yourself</a>.</div>
            ) : results.map(sub => (
...
export default function SearchPage() {
  return <Suspense fallback={<div style={{ padding: '4rem', textAlign: 'center' }}>Loading...</div>}><SearchResults /></Suspense>
}
